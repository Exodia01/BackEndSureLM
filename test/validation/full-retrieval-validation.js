...
import * as qdrantVector from "../../lib/retrieval/vector/index";
import * as qdrantLib from "../../lib/qdrant";
...
async function checkServices(): Promise<void> {
  ...
  try {
    ...
    const ollamaUrl = ollamaHost.endsWith("/v1") ? ollamaHost.replace("/v1", "") : ollamaHost;
      ...
      const hasEmbeddingModel = modelsData.models?.some((m: any) => m.name === "nomic-embed-text");
      ...
      throw new Error("Ollama check failed: " + err);
    endStage(stage, true);
  } catch (error) {
    endStage(stage, false, error instanceof Error ? error.message : String(error));
    await generateReport();
    process.exit(1);
async function processPDF(): Promise<string> {
  const stage = startStage("PDF Extraction");
    const fs = await import("fs");
      if (!fs.readFileSync(PDF_PATH).byteLength) {
  ...
  const stage = startStage("Retrieval Tests");
    ...
    for (const query of queryTexts) {
      ...
      try {
        ...
        const vectorResult = await qdrantLib.semanticSearch(queryVector, undefined, QDRANT_COLLECTION, 3);
        results.push({
          query,
    ...
    report.results.hasRetrievalResults = results.some(r => r.ftsCount > 0 || r.vectorCount > 0);
async function cleanup(docId: string | null): Promise<void> {
  if (!docId) return;
    await qdrantLib.deletePoints(QDRANT_COLLECTION, (await db.chunk.findMany({ where: { documentId: docId } })).map(c => `${docId}_chunk_${c.chunkOrder}`));
        ...
        for (const test of report.results.retrievalTests) {
          ...
          if (test.ftsResults && test.ftsResults.length > 0) {
            ...
            for (const r of test.ftsResults) {
              markdown += `| ${r.score.toFixed(3)} | ${r.id} | ${r.chunk_id || "N/A"} | ${r.snippet || ""} |\n`;
            }
          } else {
