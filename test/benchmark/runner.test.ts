...
|--------|-------|
| Total Queries Evaluated | ${results.summary.total_queries} |
...
| Avg Recall@5 | ${results.summary.avg_recall_at_5}% |
| Avg Reciprocal Rank (MRR) | ${results.summary.avg_reciprocal_rank} |
| Avg Rerank Improvement | ${results.summary.avg_rerank_improvement} |
| Avg Citation Accuracy | ${results.summary.avg_citation_accuracy} |
| Avg Groundedness Score | ${results.summary.avg_groundedness} |

### Latency Breakdown (ms)
| Phase | Average |
|-------|---------|
| Total Time | ${results.summary.avg_total_latency_ms} |
| Retrieval | ${results.summary.avg_retrieval_latency_ms} |
| Reranking | ${results.summary.avg_rerank_latency_ms} |
| LLM Generation | ${results.summary.avg_llm_latency_ms} |
...
`);
  ...
  lines.push("");
  lines.push("| Category | Count | Pass Rate | Recall@5 | MRR | Citation Acc | Groundedness |
|----------|-------|-----------|----------|-----|--------------|---------------|");
  ...
  for (const category of categories) {
    ...
    lines.push(
      `| ${category} | ${cat.count} | ${
        ...
        cat.avg_reciprocal_rank
      } | ${cat.avg_citation_accuracy} | ${cat.avg_groundedness} |`
    );
  }

  lines.push("");
  lines.push("### Quality Metrics Distribution");
  lines.push("");

  const recallCounts = results.queryResults.filter(r => r.success && r.metrics.recall_at_5).length;
  const mrrHigh = results.queryResults.filter(r => r.success && (r.metrics.reciprocal_rank || 0) >= 0.5).length;
  const citationAccHigh = results.queryResults.filter(r => r.success && (r.metrics.citation_accuracy || 0) >= 0.7).length;
  const groundedHigh = results.queryResults.filter(r => r.success && (r.metrics.groundedness_score || 0) >= 0.7).length;

  lines.push(`| Metric | High Performance (≥0.7) | Medium (0.5-0.7) | Low (<0.5) |
|--------|------------------------|------------------|------------|
| Citation Accuracy | ${citationAccHigh} | ${results.summary.total_queries - citationAccHigh - results.queryResults.filter(r => !r.success).length} | ${results.queryResults.filter(r => r.success && (r.metrics.citation_accuracy || 0) < 0.5).length} |
| Groundedness | ${groundedHigh} | ${results.summary.total_queries - groundedHigh - results.queryResults.filter(r => !r.success).length} | ${results.queryResults.filter(r => r.success && (r.metrics.groundedness_score || 0) < 0.5).length} |
...
| Success | ${r.success ? "✓" : "✗"} |
| Recall@5 | ${r.metrics.recall_at_5 ? "Yes" : "No"} |
| MRR | ${r.metrics.reciprocal_rank.toFixed(3)} |
| Error | ${r.error || "N/A"} |

**Response:** ${r.responseContent?.substring(0, 200) || "N/A"}
`);
  }
  ...
  lines.push(`| Metric | Value |
|--------|-------|
| Avg Context Count Returned | ${avgContextCount.toFixed(2)} |
| Top-1 Citation Accuracy | ${results.summary.avg_citation_accuracy} |
| Groundedness Score | ${results.summary.avg_groundedness} |
...
describe("Benchmark Runner", () => {
  it("should have proper test structure with vitest imports", () => {
  ...
  it("should export the required functions", async () => {
    ...
    expect(typeof evaluateQuery).toBe("function");
