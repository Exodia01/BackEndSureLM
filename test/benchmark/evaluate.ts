...
const QUERIES_FILE = "test/benchmark/queries.json";
const RESULTS_FILE = "test/benchmark/results.json";
const REPORT_FILE = "test/benchmark/report.md";
...
function generateReport(results: BenchmarkResults): string {
  ...
  for (const category of categories) {
    ...
    lines.push(
      "| " + category + " | " + cat.count + " | " +
      cat.pass_rate + "% | " + cat.avg_recall_at_5 + "% | " +
      cat.avg_reciprocal_rank + " | " + cat.avg_citation_accuracy + " | " + cat.avg_groundedness + " |"
    );
  }
  ...
  lines.push("|--------|------------------------|------------------|------------|");
  lines.push(
    "| Citation Accuracy | " + citationAccHigh + " | " + 
    (results.summary.total_queries - citationAccHigh - results.queryResults.filter(r => !r.success).length) + " | " + 
    results.queryResults.filter(r => r.success && (r.metrics.citation_accuracy || 0) < 0.5).length + " |"
  );
  lines.push(
    "| Groundedness | " + groundedHigh + " | " + 
    (results.summary.total_queries - groundedHigh - results.queryResults.filter(r => !r.success).length) + " | " + 
    results.queryResults.filter(r => r.success && (r.metrics.groundedness_score || 0) < 0.5).length + " |"
  );

  lines.push("");
  lines.push("## Top 20 Failing Queries");
  lines.push("");
  ...
  for (let i = 0; i < Math.min(failingQueries.length, 20); i++) {
    ...
    lines.push("");
    lines.push("**Response:** " + (r.responseContent?.substring(0, 200) || "N/A"));
    lines.push("");
  }
