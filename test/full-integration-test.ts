...
async function runTests() {
  ...
  try {
    ...
    assert(ftsResults2.length === 0, "FTS should NOT find chunks when query does not match content");
    ...
    try {
      ...
      try {
        ...
        if (upsertErr.message && upsertErr.message.includes("Failed to fetch")) {
          logMessage("[INFO] Qdrant not available, skipping upsert test");
        } else {
          throw upsertErr;
        }
      }
      ...
      if (searchResults1.length > 0) {
        ...
        logMessage("[INFO] No results from Qdrant search, skipping payload test");
      }
      ...
      try {
        ...
        if (!filterErr.message || !filterErr.message.includes("Failed to fetch")) {
          throw filterErr;
        }
      }
