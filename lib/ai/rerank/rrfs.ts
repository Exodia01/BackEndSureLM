...
type RRFSResult = RerankResult & { rerankedScore: number; relevanceRank: number };
...
export function applyRRFS(results: RetrievalResult[]): RerankResult[] {
  ...
  const rrfsResults: RRFSResult[] = results.map((result) => {
    const k = 60;
  ...
  rrfsResults.sort((a, b) => {
    ...
    const sourcePriority = { fts: 0, vector: 1, history: 2 };
