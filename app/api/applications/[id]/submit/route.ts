...
import { submitApplication, ApplicationError } from "@/lib/applications/lifecycle";

/**
 * POST /api/applications/:id/submit
 * Transitions DRAFT → SUBMITTED (idempotent when already submitted).
 * Ownership is enforced inside the lifecycle service (application → lead → agentId).
 */
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const rateLimit = checkRateLimit(user.sub, "applications:submit");
    if (!rateLimit.allowed) return rateLimitExceeded("applications:submit", rateLimit.retryAfterSeconds);
    ...
    const result = await submitApplication({ applicationId: id, agentId: userInDb.id });
